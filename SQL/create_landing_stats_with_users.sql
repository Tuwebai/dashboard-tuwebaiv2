-- Script para crear tablas de estadísticas de landing page con user_id válidos
-- Solo crea las tablas necesarias sin referencias a tablas inexistentes
...
-- Insertar encuestas de satisfacción usando user_id válidos
INSERT INTO satisfaction_surveys (user_id, company_name, rating, is_satisfied, feedback) 
SELECT 
  u.id,
  companies.company_name,
  companies.rating,
  companies.is_satisfied,
  companies.feedback
FROM users u
CROSS JOIN (
  VALUES 
    ('TechStart', 5.0, true, 'Excelente plataforma, muy fácil de usar'),
    ('InnovateLab', 5.0, true, 'Ahorramos mucho tiempo con las automatizaciones'),
    ('ScaleUp', 4.8, true, 'Muy buena herramienta, algunas mejoras menores'),
    ('DataFlow', 5.0, true, 'ROI increíble, la recomiendo'),
    ('CloudTech', 4.9, true, 'Las predicciones de IA son muy precisas')
) AS companies(company_name, rating, is_satisfied, feedback)
WHERE u.role = 'admin'
LIMIT 5
ON CONFLICT DO NOTHING;

-- Si no hay usuarios admin, insertar sin user_id
INSERT INTO satisfaction_surveys (company_name, rating, is_satisfied, feedback) 
SELECT * FROM (
  VALUES 
    ('TechStart', 5.0, true, 'Excelente plataforma, muy fácil de usar'),
    ('InnovateLab', 5.0, true, 'Ahorramos mucho tiempo con las automatizaciones'),
...
WHERE NOT EXISTS (SELECT 1 FROM satisfaction_surveys)
ON CONFLICT DO NOTHING;
