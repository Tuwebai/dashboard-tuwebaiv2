...
CREATE TABLE public.ai_settings (
  ...
  CONSTRAINT ai_settings_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.automation_reports (
  ...
  type character varying NOT NULL CHECK (type::text = ANY (ARRAY['weekly'::character varying, 'monthly'::character varying]::text[])),
...
CREATE TABLE public.daily_tasks (
  ...
  CONSTRAINT daily_tasks_created_by_fkey FOREIGN KEY (created_by) REFERENCES public.users(id),
  CONSTRAINT daily_tasks_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id)
);
CREATE TABLE public.demo_usage (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT demo_usage_pkey PRIMARY KEY (id)
);
...
CREATE TABLE public.escalation_rules (
  ...
  action character varying NOT NULL CHECK (action::text = ANY (ARRAY['notify_manager'::character varying, 'reassign_task'::character varying, 'create_meeting'::character varying, 'send_alert'::character varying]::text[])),
  priority_threshold integer DEFAULT 3,
  days_overdue_threshold integer DEFAULT 3,
  is_active boolean DEFAULT true,
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
...
CREATE TABLE public.performance_metrics (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  productivity_increase numeric DEFAULT 0,
  time_saved_hours numeric DEFAULT 0,
  meeting_time_reduction numeric DEFAULT 0,
  prediction_accuracy numeric DEFAULT 0,
  decision_speed_multiplier numeric DEFAULT 0,
  project_delay_reduction numeric DEFAULT 0,
...
CREATE TABLE public.project_approval_requests (
  ...
  CONSTRAINT project_approval_requests_reviewed_by_fkey FOREIGN KEY (reviewed_by) REFERENCES public.users(id),
  CONSTRAINT project_approval_requests_requested_by_fkey FOREIGN KEY (requested_by) REFERENCES public.users(id)
);
...
CREATE TABLE public.projects (
  ...
  CONSTRAINT projects_approved_by_fkey FOREIGN KEY (approved_by) REFERENCES public.users(id),
...
CREATE TABLE public.satisfaction_surveys (
  ...
  CONSTRAINT satisfaction_surveys_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id)
);
...
CREATE TABLE public.websy_conversation_memories (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid,
...
CREATE TABLE public.websy_knowledge_base (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid,
  title text NOT NULL,
  content text NOT NULL,
...
CREATE TABLE public.websy_learning_patterns (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid,
  ...
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT websy_learning_patterns_pkey PRIMARY KEY (id),
  CONSTRAINT websy_learning_patterns_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id)
);
CREATE TABLE public.websy_project_contexts (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid,
  project_id uuid,
  context_summary text NOT NULL,
  key_technologies ARRAY DEFAULT '{}'::text[],
  business_goals ARRAY DEFAULT '{}'::text[],
  technical_requirements ARRAY DEFAULT '{}'::text[],
  stakeholders ARRAY DEFAULT '{}'::text[],
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  CONSTRAINT websy_project_contexts_pkey PRIMARY KEY (id),
  CONSTRAINT websy_project_contexts_user_id_fkey FOREIGN KEY (user_id) REFERENCES auth.users(id),
  CONSTRAINT websy_project_contexts_project_id_fkey FOREIGN KEY (project_id) REFERENCES public.projects(id)
);
CREATE TABLE public.websy_user_profiles (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  user_id uuid UNIQUE,
  work_patterns ARRAY DEFAULT '{}'::text[],
  preferred_communication_style text DEFAULT 'balanced'::text,
  common_tasks ARRAY DEFAULT '{}'::text[],
  expertise_areas ARRAY DEFAULT '{}'::text[],
  project_contexts ARRAY DEFAULT '{}'::text[],
